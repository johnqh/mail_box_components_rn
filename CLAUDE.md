...
This is a React Native component library (`@sudobility/components-rn`) built with TypeScript, Vite, and NativeWind. It provides reusable UI components ported from `@sudobility/components`, with design system functionality imported from `@sudobility/design`.
...
- Uses `cn()` utility for conditional class merging (clsx only, no tailwind-merge for RN)
...
- Web: `<span>` → RN: `<Text>`
- ARIA attributes → RN accessibility props (`accessibilityRole`, `accessibilityLabel`, etc.)
...
- Import design tokens: `import { designTokens, colors, variants } from '@sudobility/design'`
